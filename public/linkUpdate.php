...
function ciniki_filmschedule_linkUpdate(&$ciniki) {
	...
	if( isset($args['url']) && $args['url'] != '' ) {
		...
		if( !isset($rc['link']) ) {
			return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2489', 'msg'=>'The event link does not exist'));
		}
		$link = $rc['link'];
		...
		if( isset($rc['link']) || (isset($rc['rows']) && count($rc['rows']) > 0) ) {
			return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2487', 'msg'=>'You already have a event link with that url, please choose another'));
		}
	}
