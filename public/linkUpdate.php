...
// -------
//
function ciniki_filmschedule_linkUpdate(&$ciniki) {
    //  
    ...
    $rc = ciniki_filmschedule_checkAccess($ciniki, $args['business_id'], 'ciniki.filmschedule.linkUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    ...
    if( isset($args['url']) && $args['url'] != '' ) {
        ...
        if( !isset($rc['link']) ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.filmschedule.21', 'msg'=>'The event link does not exist'));
        }
        $link = $rc['link'];
        ...
        if( isset($rc['link']) || (isset($rc['rows']) && count($rc['rows']) > 0) ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.filmschedule.22', 'msg'=>'You already have a event link with that url, please choose another'));
        }
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['business_id'], 'ciniki.filmschedule.link', 
