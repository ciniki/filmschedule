...
function ciniki_filmschedule_linkAdd(&$ciniki) {
    ...
    $rc = ciniki_filmschedule_checkAccess($ciniki, $args['business_id'], 'ciniki.filmschedule.linkAdd'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    ...
    if( isset($rc['link']) || (isset($rc['rows']) && count($rc['rows']) > 0) ) {
        return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'2486', 'msg'=>'You already have a event link with that url, please choose another'));
    }
    ...
    $rc = ciniki_core_objectAdd($ciniki, $args['business_id'], 'ciniki.filmschedule.link', $args, 0x07);
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    $link_id = $rc['id'];
